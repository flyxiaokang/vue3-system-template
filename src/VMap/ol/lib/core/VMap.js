...
class VMap extends Base {
  ...
  registerProj() {
    ...
    // proj4.defs("EPSG:4326", "+proj=longlat +ellps=WGS84 +no_defs");
    // 注册到OpenLayers
    register(proj4);
  }
  ...
  initMap(
    ...
    const map = new Map({
      ...
      target: this.target,
      view: new View({ ...view, projection: prj }),
    });
    this.map = map;
  ...
  getBaseLayer(group) {
    ...
    group.children.forEach((l) => {
      const { id, visible, opacity, type, url } = l;
      if (this.checkLayer(l)) {
        let layerInfo = l;
        if (type === V_MAP_PROVIDER.tdt && !url) {
          layerInfo = {
            type,
  ...
  getLayerByType(options, prj = this.mapConfig.prj) {
    ...
    switch (options.type) {
      case V_MAP_PROVIDER["wmts-xml"]:
        layer = this.layerHandler.getWmtsByCapabilities(options);
        break;
      case V_MAP_PROVIDER.wmts:
        layer = this.layerHandler.getWmtsByPrj({
          prj,
          options,
        });
        break;
      case V_MAP_PROVIDER.tdt:
        layer = this.layerHandler.getTdtByPrj({
          prj,
          options,
        });
        break;
      case V_MAP_PROVIDER.supermapwmts:
        layer = this.layerHandler.getSuperMapWmts({
          prj,
          options,
        });
        break;
      case V_MAP_PROVIDER.supermaprest:
        layer = this.layerHandler.getSuperMapRest({
          prj,
          options,
        });
        break;
      case V_MAP_PROVIDER.geoserverwmts:
        // layer = this.layerHandler.getWmtsGeoserver({
        //   options,
        // })
        break;
      case V_MAP_PROVIDER.wmsimage:
        layer = this.layerHandler.getWmsImage(options);
        break;
      case V_MAP_PROVIDER.wmsimagetile:
        layer = this.layerHandler.getWmsImageTile(options);
        break;
      case V_MAP_PROVIDER.arcgisimage:
        layer = this.layerHandler.getArcgisImage(options);
        break;
      case V_MAP_PROVIDER.arcgisimagetile:
        layer = this.layerHandler.getArcgisImageTile(options);
        break;
      case V_MAP_PROVIDER.arcgistile:
        layer = this.layerHandler.getXYZ(options);
        break;
      case V_MAP_PROVIDER.geojson:
        layer = this.layerHandler.getGeojsonLayerWithRender(options);
        break;
      case V_MAP_PROVIDER.heatmap:
        layer = this.layerHandler.getHeatMapLayer(options.geojson, options);
        break;
      case V_MAP_PROVIDER.clustermap:
        layer = this.layerHandler.getClusterLayerFromGeojson(
          options.geojson,
          options
        );
        break;
      ...
      case V_MAP_PROVIDER.tms:
        layer = this.layerHandler.getTmsLayer(options);
        break;
      case V_MAP_PROVIDER.gdmap:
        layer = this.layerHandler.getGaodeLayer(options);
        break;
      case V_MAP_PROVIDER.bdmap:
        layer = this.layerHandler.getBaiduLayer(options);
        break;
      case V_MAP_PROVIDER.geoservermvt:
        layer = this.layerHandler.getMvt({ prj, options });
        break;
      case V_MAP_PROVIDER.mapboxmvt:
        layer = this.layerHandler.getMapboxVt(options);
        break;
      case V_MAP_PROVIDER.supermapstyle:
        layer = this.layerHandler.getSuperMapStyle({prj,options});
        break;
      default:
        return null;
    }
    ...
    zIndex !== undefined && layer.setZIndex(zIndex);
    minZoom !== undefined && layer.setMinZoom(minZoom);
    maxZoom !== undefined && layer.setMaxZoom(maxZoom);
    return layer;
  }
  ...
  parseLayerOptions(o) {
    ...
    if (o.hasOwnProperty("zIndex") && o.zIndex !== undefined) {
  ...
  addWmsLayer(options) {
    this.map.addLayer(this.layerHandler.getWmsImageTile(options));
  }
  ...
  highlightGeometry(
    ...
    if (typeof geometry === "string") {
      geometry = new WKT().readFeature(geometry);
    } else {
      geometry = new GeoJSON().readFeature(geometry);
    }
    const feature = new Feature(geometry);
    const layer = this.layerHandler.getVectorLayer();
    layer.getSource().addFeature(feature);
    layer.setStyle(StyleHandler.getStyle(options.style));
    map.addLayer(layer);
   ...
   * @returns
   */
  getArea(obj, projection = "EPSG:4326") {
    const geom =
      typeof obj === "string"
        ? new WKT().readGeometry(obj)
        : new GeoJSON().readGeometry(geojson);
    const area = getArea(geom, { projection });
    return Number(area / 1000000);
  }
}
