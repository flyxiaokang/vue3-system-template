 ...
 * @LastEditTime: 2024-02-02 14:55:40
-->
<template></template>
...
import { getConfig } from '@/VMap/cesium/config'
...
const getLayerId = computed(() => {
  if (isTdt.value) {
    return (mapProvider.value + '_' + mapStyle.value).toLowerCase()
  } else {
...
const getArcgisImageTile = () => {}

const initLayer = () => {
  console.log(getUrl.value)
  const type = V_MAP_PROVIDER[getMapProvider.value]
  if (!type) {
    console.error('<vmap>未知的地图提供者!')
    return
  }
  const id = getLayerId.value
  if (getUrl.value && checkUrl()) {
    cesiumHandler.removeLayerById(id)
    // if (isWmts.value) {
    //   layerObject.value = cesiumHandler.loadLayer({
    //     ...props,
    //     id,
    //     type,
    //     url: getUrl.value,
    ...
    //   })
    // } else if (isArcgisImage.value || isArcgisImageTile.value) {
    //   layerObject.value = cesiumHandler.loadLayer({
    //     ...props,
    //     id,
    //     type,
    ...
    // cesiumHandler.map.addLayer(layerObject.value)
    console.log('layer ready.........', layerObject.value)
...
onUnmounted(() => {
  cesiumHandler && cesiumHandler.removeLayerById(getLayerId.value)
})
</script>
