...
scene.on('loaded', () => {
  ...
  getNcovData().then((ncovData => {
    ...
    // popup.setLnglat(['120', '33']).setHTML('<span>1212</span>').addTo(scene);
  }))
...
async function getNcovData() {
  ...
  for (let i = 0; i < data.data.getAreaStat.length; i++) {
    ...
    if (item.provinceShortName == '江苏') {
      window.defaultProvice = provinceData[i]
    }
    // console.log("test>>>", window.defaultProvice)
    // }
